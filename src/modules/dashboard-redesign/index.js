...
import { RecentSuggestionsComponent } from './components/recent-suggestions.js';

export default class DashboardRedesignModule {
    constructor() {
        this.name = 'dashboard-redesign';
        this.container = null;
        ...
        // Componente de sugerencias recientes
        this.recentSuggestions = null;
    ...
    async load(container) {
        ...
        try {
            ...
            // 7. Inicializar componente de sugerencias recientes
            this.initializeRecentSuggestions();
            ...
            // 10. Emitir evento de módulo cargado
            this.eventBus.emit('module:loaded', { module: this.name });
            
        } catch (error) {
            console.error('[Dashboard Redesign] Error loading module:', error);
     ...
     */
    async loadTemplate() {
        const response = await fetch('/modules/dashboard-redesign/template.html');
        if (!response.ok) {
    ...
    async loadVoices() {
        try {
            ...
            if (defaultVoice) {
                ...
                console.log('[Dashboard Redesign] Voz por defecto establecida:', defaultVoice.label);
            ...
            console.error('[Dashboard Redesign] Error loading voices:', error);
    ...
    /**
     * Inicializar componente de sugerencias recientes
     */
    initializeRecentSuggestions() {
        const placeholder = document.getElementById('recent-suggestions-placeholder');
        if (placeholder) {
            this.recentSuggestions = new RecentSuggestionsComponent(this);
            this.recentSuggestions.init(placeholder);
        }
    }
    ...
    async handleGenerate() {
        ...
        try {
            ...
            console.error('[Dashboard Redesign] Generation error:', error);
    ...
    async loadRecentMessages() {
        try {
            ...
            console.error('[Dashboard Redesign] Error loading messages:', error);
            ...
            <div class="message-item" data-id="${msg.id}">
                ...
                <div class="message-footer">
                    <div class="message-actions">
                        ${msg.filename ? `<button class="btn-icon" title="Reproducir" onclick="window.dashboardRedesign.playMessageAudio('${msg.filename}')">▶</button>` : ''}
                        <button class="btn-icon btn-save" title="Guardar" onclick="window.dashboardRedesign.saveToFavorites('${msg.id}', '${msg.filename || ''}', '${(msg.title || '').replace(/'/g, "\\'")}', '${msg.category || 'sin_categoria'}')">✓</button>
                        <button class="btn-icon btn-delete" title="Eliminar" onclick="window.dashboardRedesign.removeMessage('${msg.id}')">🗑️</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    async unload() {
        ...
        if (dashboardStyles) {
            ...
            console.log('[Dashboard Redesign] Styles removed');
        }
    ...
    showSuccess(message) {
        ...
        console.log('[Dashboard Redesign] Success:', message);
    }
    
    showError(message) {
        // Aquí podrías implementar un toast notification
        console.error('[Dashboard Redesign] Error:', message);
    ...
    updateCategory(category) {
        ...
        console.log('[Dashboard Redesign] Categoría actualizada a:', category);
    }
    ...
    initializeAISuggestions() {
        try {
            ...
            // Montar el componente - el método mount busca automáticamente 'ai-suggestions-placeholder'
            this.aiSuggestions.mount();
            ...
            this.eventBus.on('llm:suggestion:selected', (data) => {
                ...
                console.log('[Dashboard Redesign] Sugerencia seleccionada:', data.suggestion.text.substring(0, 50) + '...');
            });
            
            console.log('[Dashboard Redesign] Componente IA inicializado');
            
        } catch (error) {
            console.error('[Dashboard Redesign] Error inicializando IA:', error);
