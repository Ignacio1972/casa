...
export class AISuggestionsComponent {
                ...
                <div class="ai-config-body">
                    ...
                    <div class="ai-advanced">
                        <button 
                            ...
                            onclick="window.dashboardRedesign.aiSuggestions.toggleAdvanced()"
                        >
                    ...
                    <button 
                        ...
                        onclick="window.dashboardRedesign.aiSuggestions.generate()"
        ...
        return `
            <div id="aiSuggestionsPanel" class="ai-suggestions-panel">
                <div class="ai-suggestions-header">
                    ...
                    <button 
                        class="btn-text"
                        onclick="window.dashboardRedesign.aiSuggestions.clearSuggestions()"
                    >
                        Limpiar
                    </button>
                </div>
            ...
            <div class="ai-suggestion-card ${isSelected ? 'selected' : ''}" data-suggestion-id="${suggestion.id}">
                ...
                <div class="suggestion-actions">
                    <button 
                        ...
                        onclick="window.dashboardRedesign.aiSuggestions.useSuggestion('${suggestion.id}')"
                    ...
                    <button 
                        ...
                        onclick="window.dashboardRedesign.aiSuggestions.regenerateSuggestion('${suggestion.id}')"
                    ...
                    <button 
                        ...
                        onclick="window.dashboardRedesign.aiSuggestions.copySuggestion('${suggestion.id}')"
    ...
    handleSuggestionsGenerated(data) {
        ...
        // Guardar sugerencias en la BD separada (NO afecta producción)
        this.saveSuggestionsToDatabase(data);
    }
    
    /**
     * Guardar sugerencias en la BD separada
     */
    async saveSuggestionsToDatabase(data) {
        try {
            // Obtener el contexto actual
            const context = document.getElementById('aiContext')?.value || '';
            const tone = document.getElementById('aiTone')?.value || 'profesional';
            const duration = document.getElementById('aiDuration')?.value || 30;
            
            const response = await fetch('/api/save-ai-suggestions.php', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                console.log('[AI Suggestions] Saved to separate database:', result.saved);
            }
        } catch (error) {
            console.error('[AI Suggestions] Error saving to database:', error);
            // No mostrar error al usuario, es una función secundaria
        }
    }
    ...
    mount(containerId) {
        ...
        console.log('[AI Component] Mounting AI suggestions, placeholder found:', !!placeholder);
        
        if (placeholder) {
            // Reemplazar el placeholder con el componente AI
            placeholder.innerHTML = this.render();
            console.log('[AI Component] AI suggestions rendered');
